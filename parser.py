...
from save_to_db import save_mysql_channel
...
fname = r"D:\python\parser_2\parser\mhtlm_files\messages9.html"
...
a = prepare_group_info(result)  # Здесь происходит первичная обработка данных
dict_reply = get_from_name_joined(result)  # Здесь мы получаем словарь с replied_id и from_name
g = get_from_name(a, dict_reply)  # Данная функция возвращает на выходе готовый список данных из Learning Group для отправки в бд
